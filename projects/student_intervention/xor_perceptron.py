...
def EvalNetwork(inputValues, Network):
    ...
    # YOUR CODE HERE
    middleLayer = np.apply_along_axis(lambda inputs: Perceptron(inputs[0], inputs[1]).activate(inputValues), 1, Network[0])
    # Be sure your output value is a single number
    OutputValue = Perceptron(Network[1]).activate(middleLayer)
    return OutputValue
